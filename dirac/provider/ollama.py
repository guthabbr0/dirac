...
class OllamaProviderClient(BaseProviderClient):
    def __init__(self, *, base_url: str, api_key: str = "", timeout_s: float = 120.0, name: str = "ollama") -> None:
    ...
    async def chat(self, request: ProviderRequest) -> ProviderResponse:
        ...
        if request.params:
            body["options"] = {key: value for key, value in request.params.items() if key in {"temperature", "top_p", "top_k", "seed"}}
        ...
        return ProviderResponse(content=content, raw=raw, tool_calls=tool_calls, provider_name=self.name, model=request.model)
