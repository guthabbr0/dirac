...
from .contracts import is_discord_id, normalize_discord_id, parse_confidence, parse_tags, validation_error
...
class MemoryToolFacade:
    ...
    async def search(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        query = MemoryQuery(text=args.get("str_query"), discord_id=normalize_discord_id(args.get("str_discord_id")), limit=max(1, min(limit, 50)))
    ...
    async def add(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        if not is_discord_id(discord_id):
            issues.append("str_discord_id invalid or missing: provide one Discord snowflake id or mention.")
        ...
        row = await self.memory.add(MemoryWrite(discord_id, annotations, tags, confidence, self.created_by))
    ...
    async def update(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        row = await self.memory.update(memory_id, MemoryPatch(str(args.get("str_annotations") or ""), tags, confidence, self.created_by))
        return {"ok": True, "memory": asdict(row)}
