...
class ConsolidationService:
    ...
    def __init__(self, *, memory: BaseMemory, provider: BaseProviderClient, events: VisibleEventSource) -> None:
        self.memory = memory
        self.provider = provider
        self.events = events

    async def consolidate(self, *, scope: Scope, model: str, minutes: int = 10) -> ConsolidationAudit:
        ...
        request = ProviderRequest(
            messages=[{"role": "user", "content": "Consolidate these visible events into durable memory candidates."}],
            model=model,
            scope=scope,
        ...
        return ConsolidationAudit(True, response.content, detail={"event_count": len(visible_events)})
