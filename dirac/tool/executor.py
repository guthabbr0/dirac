...
class ToolExecutionService:
    ...
    async def run_enabled_tool(self, name: str, args: dict[str, Any], scope: Scope, enabled_names: set[str]) -> ToolResult:
        if name not in enabled_names:
            return ToolResult(name, False, {"error": "tool_not_enabled_or_unknown"}, needs_model_followup=True)
